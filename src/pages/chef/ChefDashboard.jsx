...
const saveNotifiedOrders = (setObj) => {
  localStorage.setItem("shownNotifications", JSON.stringify([...setObj]));
};
...
export default function ChefDashboard() {
  ...
  const handleNewOrder = useCallback(
    (order) => {
      ...
      setRecent((prev) =>
        [order, ...prev.filter((o) => o.orderCode !== order.orderCode)].slice(0, 3)
      );
  ...
  // Only listen here — connection handled in App.jsx
  useEffect(() => {
    if (!chefId) return;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        sortedRecent.forEach((order) => {
          if (!notifiedOrdersRef.current.has(order.orderCode)) {
                ...
                Notification.requestPermission().then((permission) => {
                  if (permission === "granted") {
                    new Notification("Notifications enabled!", {
                      body: "You will now receive new order alerts.",
                    });
                  }
                });
              }}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4 mt-2 sm:mt-0">
                        ...
                        <span className="text-gray-500 text-xs sm:text-sm">{fmtWhen(createdAt)}</span>
                      </div>
                    </div>
                  );
                })}
