...
const FoodItemCard = ({ item }) => {
  ...
  e.stopPropagation();
  if (!item.available) return;            // 🔹 no-op if sold out
  addToCart(item);
  setIsCartOpen(true);
};
  ...
  return (
      <>
      <div
        onClick={!isSoldOut ? () => navigate(`/dish/${item.slug}`) : undefined}
        className={`bg-white rounded-xl border [border-color:#e0dada] hover:border-primary/40 hover:scale-[1.01] transition-transform duration-50 ease-in-out w-full max-w-[270px] mx-auto flex flex-col ${!isSoldOut && "cursor-pointer"}`}
      >
        {/* Image */}
        ...
        <div className="p-4 flex flex-col flex-grow">
          ...
          {/* Price + Button — keep your exact available-state UI */}
          <div className="flex items-center justify-between mt-auto pt-2">
            <p className="text-sm font-bold text-primary">
              ${item.price ? item.price.toFixed(2) : "0.00"}
            </p>

            {/* Render TWO different buttons to avoid hover/opacity bugs */}
            {!isSoldOut ? (
              <button
