...
export default function PostFood() {
  ...
  useEffect(() => {
    const getKtererAccountInfo = async () => {
      ...
      try {
        ...
        // New check is added to identify if the kterer stripe account was created correctly.
        let client_id = data.user.client_id;

        if (data.user.kterer.stripe_account_id !== null && client_id) {
          try {
            const params = new URLSearchParams({
              client_id: client_id.toString(),
            });
            const response = await fetch(
              `${apiURL}/api/stripe/verifyAccount?${params}`,
              {
                method: "GET",
                headers: {
                  "Content-Type": "application/json",
  ...
  const getButtonClass = (isActive: boolean) =>
    `rounded-full px-4 py-2 text-center w-full ${
      isActive ? "bg-primary-color text-white" : "bg-gray-200"
    }`;
          ...
          {!ktererInfo && (
            <>
              <Alert variant="destructive">
                ...
                <AlertDescription>
                  Please complete your banking information to enable food
                  posting.{" "}
                  <Link href="/kterer/earnings" className="underline">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>
                        Tell us a little bit about your food item. Is it perfect
                        for dinner? Lunch? Is it good for 3 people? Let
                        customers know!
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>
                        Kosher is food prepared according to the requirements of
                        Jewish law.
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  ...
                  render={({ field }) => (
                    <div className="bg-primary-color px-3.5 py-2  text-white">
                      ...
                      <h3 className="font-light italic mb-2">
                        Important! This will help determine when your food will
                        be picked up for delivery so make it as accurate as you
                        can!
                      </h3>
                      <div className="flex justify-center items-center mb-1">
                        <h3 className="inline-block grow font-medium">
                          How long do you need to prepare/make this item?
                        </h3>
                        <FormItem className="inline-block text-black">
                          <Select
                            onValueChange={field.onChange}
